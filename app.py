...
CONTACT_SYSTEM_PROMPT = (
    ...
    '"t":"<title or empty>","u":"<source URL or empty>","l":"<C|H|U>"}. '
    "Status codes: "
...
def apply_contact_result(row, result: dict) -> dict:
    ...
    # Grok saw the premises while searching, so its read overrides our guess.
...
def accept_smarty_contact(row) -> dict:
    """
    Turn a Smarty business contact into a resolved contact, no search needed.

    Smarty's record is a maintained database rather than a web guess, so it
    stands on its own. Marked Verified when it agrees with the list and
    Updated when it replaces what the list said.
    """
    contact = blank_contact()
    ...
    contact.update({
        ...
        "Contact Note": (
            "Smarty confirms the listed contact" if agrees
            else (f"Smarty has {first} {last}".strip() +
                  (f", replacing {our_first} {our_last}".strip() if our_last else
                   " - list had no name"))
        ),
    })
    return contact
...
def resolve_contacts(df: pd.DataFrame, api_key: str, limit: int, rates: dict,
                     ...
                     use_smarty: bool = True, progress=None):
    """
    ...
    for index in targets:
        ...
        # Smarty already answered this one - no search, no spend.
        if use_smarty and has_smarty:
            smarty_last = str(df.at[index, "Smarty Contact Last Name"] or "").strip()
            if smarty_last and smarty_last.lower() not in ("none", "nan"):
                results[index] = accept_smarty_contact(df.loc[index])
...
def stage_contacts(working_df: pd.DataFrame, file_key: str):
    ...
    st.subheader("4. Decision maker (Grok)")
    ...
    if st.button(f"Resolve {limit} companies", type="primary"):
        ...
        try:
            st.session_state[cache_key] = resolve_contacts(
                working_df, XAI_API_KEY, limit, rates, model["id"], use_smarty, progress
            )
        except Exception as err:
...
def main() -> None:
    ...
    st.title(APP_TITLE)
    st.write(
        "Four stages, in order: employee count, industry type, Smarty business "
        "data, then Grok for the decision makers Smarty didn't supply. The two "
        "free stages run first so nothing metered is spent on a lead you were "
        "going to discard."
    )
    ...
    with st.expander("How the list narrowed"):
        ...
        if RESOLVED_SOURCE in working_df.columns:
            st.caption(
                "Stages after the address step read Smarty's headcount and SIC "
                "code where it has them, falling back to your file otherwise. "
                "The 'Data source' column shows which was used per row."
            )
