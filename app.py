...
# Prefix for generated IDs. Change per territory or campaign if useful.
ROW_ID_PREFIX = "BG"


def stable_row_id(row, prefix: str) -> str:
    """
    A permanent id for one business.

    Derived from the source system's own identifier when there is one, and
    otherwise from a hash of the company name and address. Either way the same
    business gets the same id on every export, so an id never depends on where
    a row happened to sit in a filtered file.
    """
    source_id = str(row.get("Infogroup ID", "") or "").strip()
    if source_id and source_id.lower() not in ("none", "nan"):
        digits = "".join(char for char in source_id if char.isalnum())
        return f"{prefix}-{digits.upper()[:12]}"

    # No source id: hash the identity fields so re-uploads are stable.
    identity = "|".join([
        " ".join(sorted(normalize_company(row.get("Company Name", "")))),
        normalize_address(row.get(STREET_COLUMN, "")),
        str(row.get(CITY_COLUMN, "") or "").strip().lower(),
        str(row.get(STATE_COLUMN, "") or "").strip().lower(),
    ])
    digest = hashlib.sha1(identity.encode("utf-8")).hexdigest()[:10]
    return f"{prefix}-{digest.upper()}"


def add_row_identity(df: pd.DataFrame, prefix: str = ROW_ID_PREFIX) -> pd.DataFrame:
    """
    Give every row a permanent Source Row ID and normalized keys.

    An existing Source Row ID column is preserved untouched - re-uploading a
    file that has already been through the app must not renumber anything.
    Collisions (two genuinely different rows hashing alike) get a suffix so
    the ids stay unique, which the merge depends on.
    """
    result = df.copy()
    ...
    existing = result.get("Source Row ID")
    if existing is not None and existing.fillna("").astype(str).str.strip().ne("").all():
        # Already identified - leave the ids exactly as they are.
        return result

    ids, seen = [], {}
    for _, row in result.iterrows():
        base = stable_row_id(row, prefix)
        if base in seen:
            seen[base] += 1
    ...
    if "Source Row ID" in result.columns:
        result = result.drop(columns=["Source Row ID"])
    result.insert(0, "Source Row ID", ids)

    return result


def make_batch_id(prefix: str = ROW_ID_PREFIX) -> str:
    """
    An id for one handoff export, e.g. BG_2026_09_12_001.

    Written onto every row so a returned file can be checked against the
    export it came from.
    """
    from datetime import date

    today = date.today().strftime("%Y_%m_%d")
    counter = st.session_state.get("batch_counter", 0) + 1
    st.session_state["batch_counter"] = counter
    return f"{prefix}_{today}_{counter:03d}"
...
HANDOFF_FILENAME = "CHATGPT_RESEARCH_HANDOFF.csv"
RETURN_FILENAME = "CHATGPT_RESEARCH_RETURN.csv"

# Research columns, blank on export, populated by ChatGPT on the same rows.
RESEARCH_COLUMNS = [
    "Research Identity Status",
    "Research Identity Note",
    "Research Business Status",
    ...
    "Parent Company",
    "Parent HQ",
    "Benefits Decision Level",
    "Executive Conflict Note",
...
# Identity columns lead the file, in this order.
HANDOFF_LEAD_COLUMNS = [
    "Source Row ID",
    "Handoff Batch ID",
    "Company Name",
    STREET_COLUMN,
...
# Working columns that would only confuse the research pass.
HANDOFF_DROP_COLUMNS = {
    "Normalized Company", "Normalized Address", "Duplicate Group ID",
    "_size_sort", "_tier_sort",
}


def build_handoff(df: pd.DataFrame, batch_id: str) -> pd.DataFrame:
    """
    Assemble the CSV ChatGPT receives.

    Keeps every structured and enriched field - they are clues for identity
    verification and the basis for conflict detection - then appends the blank
    research columns. Identity columns lead so the merge key is the first
    field in the file.
    """
    handoff = df.copy()
    handoff["Handoff Batch ID"] = batch_id

    # Research columns must already exist, and must be blank on export. If a
    # previous pass populated one, it is carried through rather than wiped.
    for name in RESEARCH_COLUMNS:
        if name not in handoff.columns:
            handoff[name] = ""

    handoff = handoff.drop(
        columns=[name for name in HANDOFF_DROP_COLUMNS if name in handoff.columns]
    )

    lead = [name for name in HANDOFF_LEAD_COLUMNS if name in handoff.columns]
    middle = [name for name in handoff.columns
              if name not in lead and name not in RESEARCH_COLUMNS]

    return handoff[lead + middle + RESEARCH_COLUMNS]


def handoff_to_csv(handoff: pd.DataFrame) -> bytes:
    """
    UTF-8 CSV, no index column.

    index=False matters: a leading unnamed column shifts every field and
    breaks the merge on the way back.
    """
    return handoff.to_csv(index=False).encode("utf-8")


def validate_return(sent: pd.DataFrame, returned: pd.DataFrame, batch_id: str):
    """
    Check a returned file before merging anything.

    Returns (errors, warnings). Errors block the merge; warnings are reported
    but allow it. The app fails safely rather than inferring a match.
    """
    errors, warnings = [], []

    if "Source Row ID" not in returned.columns:
        errors.append(
            "The returned file has no Source Row ID column. Nothing can be "
            f"merged safely - regenerate {HANDOFF_FILENAME} from this stage and "
            "send that file."
        )
        return errors, warnings

    returned_ids = returned["Source Row ID"].fillna("").astype(str).str.strip()
    sent_ids = sent["Source Row ID"].astype(str).str.strip()

    if returned_ids.eq("").any():
        errors.append(f"{int(returned_ids.eq('').sum())} returned row(s) have a blank Source Row ID.")

    duplicated = returned_ids[returned_ids.duplicated() & returned_ids.ne("")]
    if len(duplicated):
        errors.append(
            f"{len(duplicated)} duplicate Source Row ID(s) in the return "
            f"(first: {duplicated.iloc[0]}). Each must appear exactly once."
        )

    sent_set, returned_set = set(sent_ids), set(returned_ids)

    added = returned_set - sent_set - {""}
    if added:
        errors.append(
            f"{len(added)} Source Row ID(s) in the return were never sent "
            f"(first: {sorted(added)[0]})."
        )

    absent = sent_set - returned_set
    if absent:
        errors.append(
            f"{len(absent)} row(s) sent for research are missing from the return "
            f"(first: {sorted(absent)[0]}). Rows must not be dropped."
        )

    if len(returned) != len(sent):
        errors.append(
            f"Row count changed: sent {len(sent):,}, received {len(returned):,}."
        )

    # Batch ID confirms this return belongs to this export.
    if "Handoff Batch ID" in returned.columns:
        batches = set(returned["Handoff Batch ID"].fillna("").astype(str).str.strip()) - {""}
        if batches and batches != {batch_id}:
            errors.append(
                f"Batch ID mismatch - expected {batch_id}, found "
                f"{', '.join(sorted(batches))}. This return belongs to a "
                "different export."
            )
    else:
        warnings.append("No Handoff Batch ID column in the return, so the export couldn't be confirmed.")

    # Company names are a secondary check only, never a merge key.
    if "Company Name" in returned.columns and not errors:
        theirs = returned.set_index(returned_ids)["Company Name"].astype(str)
        mismatched = 0
        for key, ours in zip(sent_ids, sent["Company Name"].astype(str)):
            other = theirs.get(key)
            if other is not None and not same_company(ours, str(other)):
                mismatched += 1
        if mismatched:
            warnings.append(
                f"{mismatched} row(s) came back with a company name that doesn't "
                "match its Source Row ID. Check those before trusting the research."
            )

    present = [name for name in RESEARCH_COLUMNS if name in returned.columns]
    if not present:
        errors.append("None of the research columns are present in the returned file.")
    elif len(present) < len(RESEARCH_COLUMNS):
        warnings.append(
            f"{len(RESEARCH_COLUMNS) - len(present)} research column(s) missing "
            "from the return; those stay blank."
        )

    return errors, warnings


def merge_research(sent: pd.DataFrame, returned: pd.DataFrame):
    """
    Merge research columns back in, keyed on Source Row ID only.

    Never falls back to company name, address or row position - a wrong match
    silently attaches one company's contact to another, which is worse than
    no merge at all.
    """
    ...
    present = [name for name in RESEARCH_COLUMNS if name in returned.columns]
    lookup = returned.drop_duplicates("Source Row ID").set_index("Source Row ID")[present]

    merged = sent.copy()
    keys = merged["Source Row ID"].astype(str).str.strip()

    for name in present:
        merged[name] = keys.map(lookup[name]).fillna("")

    filled = int(
        merged[present].fillna("").astype(str)
...
CHATGPT_PROMPT = """I am uploading CHATGPT_RESEARCH_HANDOFF.csv.

This file has already been through structured cleaning, deduplication,
enrichment, employee-count resolution, SIC resolution and qualification
filtering. Do not redo any of that, and do not re-filter the list.

VALIDATE BEFORE YOU START
Check the file first. If any of the following is true, stop and tell me -
do not invent a solution:
- the Source Row ID column is missing
- any Source Row ID is blank
- duplicate Source Row IDs exist
- the research columns are missing
- this looks like a different export than a research handoff
- the same company appears more than once and the rows cannot be told apart
  by location

If Source Row ID is missing, reply exactly:
"The handoff is missing Source Row ID. I cannot safely research and return
this dataset because the results would not be reliably mergeable. Please
regenerate CHATGPT_RESEARCH_HANDOFF.csv from the app."

Never create replacement Source Row IDs.

WHAT YOU MUST NOT DO
Do not delete, add or reorder rows. Do not change Source Row ID or Handoff
Batch ID. Do not overwrite the original structured columns. Treat structured
data as clues, not confirmed truth.

FOR EACH ROW, RESEARCH AND POPULATE THE EXISTING RESEARCH COLUMNS

1. IDENTITY
Confirm the public sources refer to the same company and location as the row.
Fill Research Identity Status (Confirmed / Ambiguous / Not found) and
Research Identity Note. If several companies match, say so rather than
picking one.

2. BUSINESS STATUS
Research Business Status: Active, Closed or Unclear.

3. ADDRESSES
Physical Operating Address - where people actually work. Never a PO Box.
Corporate / HQ Address - the head office if different.
Physical = HQ? - Yes / No / Unknown.
Physical Location Type - Commercial Storefront, Professional Office,
Industrial/Warehouse, Residential/Home-Based, or Unknown.

4. EXECUTIVE AUTHORITY
Use the existing executive fields as leads, then independently verify the
CURRENT Owner, Founder, CEO, President, Managing Partner/Member or Executive
Director. Do not assume the structured record is current. Fill Verified
Executive Authority and Verified Executive Title.

5. BENEFITS TARGET - SEPARATE RESEARCH, ALWAYS
Do this even when you have already identified an owner or CEO. Search in this
order: HR Director, HR Manager, Benefits Director, Benefits Manager, Benefits
Coordinator, People/Personnel leader, Payroll Manager, Office Manager,
Business Manager, Practice Manager, Controller, Finance Manager.
Only use Executive Authority as the Benefits Target if no separate supported
administrative contact exists.
If no supported named person can be found, enter exactly: Could not find
Do not guess. Fill Benefits Target and Benefits Target Title.

6. CORPORATE STRUCTURE
Corporate Structure: independent, franchise, branch, subsidiary, division,
nonprofit, government/public entity, or part of a larger group.
If part of a larger organization, fill Parent Company and Parent HQ.
Benefits Decision Level: Local, Corporate, Shared or Unknown.

7. CONFLICTS
Where structured data and public evidence disagree, preserve both. Explain in
Executive Conflict Note, Address Conflict Note and Corporate Structure Note.
Never silently overwrite.

8. RAPPORT
Rapport Fact 1-3: specific, outreach-relevant facts - family ownership, years
in business, recent expansion, community involvement, awards, employee
ownership, leadership background, major local projects, Chamber involvement,
notable specialty. No generic filler. Leave blank rather than padding.

9. SOURCES AND CONFIDENCE
Research Sources: the URLs you relied on.
Research Confidence: 1-10, based on strength and recency of evidence.
Research Date: today's date.
Accuracy matters more than completeness. Do not guess.

RETURN
Return the same dataset as CHATGPT_RESEARCH_RETURN.csv - same rows, same
order, same Source Row IDs, same Handoff Batch ID, same original columns,
with the research columns populated on their own rows. UTF-8 CSV, no index
column. Do not return a separate research-only file that would need matching
by company name."""
...
def stage_handoff(working_df: pd.DataFrame, file_key: str) -> pd.DataFrame:
    """
    Stage 5: hand the qualified list to ChatGPT as CSV, then merge it back.

    CSV only, keyed on Source Row ID only. The merge fails safely rather than
    inferring a match from names or row order.
    """
    st.subheader("5. ChatGPT research handoff")

    if "Source Row ID" not in working_df.columns:
        st.error(
            "No Source Row ID on these rows, so a safe merge isn't possible. "
            "Re-run from the top - Stage 0 assigns the IDs."
        )
        return working_df
    ...
    st.markdown(
        ...
        "available.\n\n"
        "The rest needs judgement rather than lookups: the current owner, the "
        "benefits contact found independently, the physical site versus the "
        "mailing address, branches and franchises, conflicts, and rapport notes."
    )

    # One batch id per export, held for the session so a re-render doesn't
    # invalidate a file already sent to ChatGPT.
    batch_key = f"batch::{file_key}::{len(working_df)}"
    if batch_key not in st.session_state:
        st.session_state[batch_key] = make_batch_id()
    batch_id = st.session_state[batch_key]

    handoff = build_handoff(working_df, batch_id)
    st.session_state[f"sent::{batch_id}"] = handoff

    a, b, c = st.columns(3)
    a.metric("Companies", f"{len(handoff):,}")
    b.metric("Columns", f"{len(handoff.columns)}")
    c.metric("Batch", batch_id)

    st.download_button(
        f"Download {HANDOFF_FILENAME}",
        data=handoff_to_csv(handoff),
        file_name=HANDOFF_FILENAME,
        mime="text/csv",
        type="primary",
    )
    st.caption(
        f"This is the only file to send - it carries Source Row ID, Handoff "
        f"Batch ID and {len(RESEARCH_COLUMNS)} blank research columns. Other "
        "stages' downloads won't merge back. Send about 25 companies at a time; "
        "longer lists come back truncated or with rows quietly dropped."
    )
    ...
    st.divider()
    st.write(f"**When {RETURN_FILENAME} comes back**")

    returned_file = st.file_uploader(
        "Upload the returned CSV", type=["csv"], key=f"research_{file_key}"
    )

    if returned_file is None:
        st.caption(
            "Merging is keyed on Source Row ID alone. If the return is missing "
            "that column, regenerate the handoff rather than matching by name."
        )
        return working_df
    ...
    # A stray index column shifts every field; catch it before anything else.
    unnamed = [name for name in returned.columns if str(name).startswith("Unnamed:")]
    if unnamed:
        returned = returned.drop(columns=unnamed)
        st.warning(
            f"Dropped {len(unnamed)} unnamed index column(s) from the return."
        )

    errors, warnings = validate_return(handoff, returned, batch_id)
    ...
    if errors:
        for message in errors:
            st.error(message)
        st.caption(
            "Nothing was merged. Fix the return file or regenerate the handoff - "
            "the app won't guess at a match."
        )
        return working_df

    merged, filled = merge_research(working_df, returned)
    st.success(f"Validated and merged. {filled:,} of {len(merged):,} rows carry research.")

    if "Benefits Target" in merged.columns:
        targets = merged["Benefits Target"].fillna("").astype(str).str.strip()
        named = int(targets.ne("").sum() - targets.str.lower().eq("could not find").sum())
        st.caption(f"{named:,} companies came back with a named benefits contact.")

    if "Benefits Decision Level" in merged.columns:
        corporate = int(
            merged["Benefits Decision Level"].fillna("").astype(str)
            .str.strip().str.lower().eq("corporate").sum()
        )
        if corporate:
            st.warning(
                f"{corporate:,} companies have benefits decided at corporate, not "
                "locally. Those aren't worth a local visit."
            )
