...
def stage_two_business(working_df: pd.DataFrame, file_key: str) -> pd.DataFrame:
    """
    Stage 2, Enrichment mode: pull Smarty's business record for each company.
    ...
    # Smarty's headcount is more current than a purchased list's range, so
    # show where they disagree rather than silently overwriting.
    if diagnostics["matched"]:
        with st.expander("Smarty data vs your file"):
            st.dataframe(
                enriched[[
                    "Company Name", SIZE_COLUMN, "Smarty Employee Count",
                    "Executive First Name", "Executive Last Name",
                    "Smarty Contact First Name", "Smarty Contact Last Name",
                    "Smarty Contact Title", "Business Status",
                ]].head(50),
                use_container_width=True, hide_index=True,
            )
            st.caption(
                "Check a few rows against what you know before relying on this. "
                "Where Smarty has a contact, stage 4 has less to search for."
            )
...
def main() -> None:
    ...
    st.divider()
    st.subheader("2. Address and business data")
    ...
    if not SMARTY_AVAILABLE:
        ...
        if smarty_mode == SMARTY_MODE_STREET:
            ...
            working_df = stage_two_business(working_df, uploaded_file.name)
